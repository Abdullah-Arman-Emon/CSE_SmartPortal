...
// ---------------------------------------------------------------------------
// Feature pages — each is its own route (own URL). They read the shared
// { user, teacherProfile, dashboardData } from the layout's Outlet context and
// feed the existing feature components their props, so no child needs editing.
// ---------------------------------------------------------------------------
  ...
  return (
    <div className="space-y-8">
          ...
          <div className="p-5 flex items-center">
            ...
            <div>
              <p className="text-3xl font-serif font-bold text-amber-700">
                {dashboardData.assignments_to_check}
              </p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <button onClick={() => navigate("create-course")} className="flex flex-col items-center gap-2 p-4 bg-white rounded-lg border border-slate-200 hover:border-amber-300 hover:shadow-md transition-all">
          <span className="text-2xl">➕</span>
          <span className="text-sm font-medium text-slate-700 font-serif">Create Course</span>
        </button>
        <button onClick={() => navigate("courses")} className="flex flex-col items-center gap-2 p-4 bg-white rounded-lg border border-slate-200 hover:border-blue-300 hover:shadow-md transition-all">
          <span className="text-2xl">📚</span>
          <span className="text-sm font-medium text-slate-700 font-serif">My Courses</span>
        </button>
        <button onClick={() => navigate("meetings")} className="flex flex-col items-center gap-2 p-4 bg-white rounded-lg border border-slate-200 hover:border-emerald-300 hover:shadow-md transition-all">
          <span className="text-2xl">📅</span>
          <span className="text-sm font-medium text-slate-700 font-serif">Meetings</span>
        </button>
        <button onClick={() => navigate("profile")} className="flex flex-col items-center gap-2 p-4 bg-white rounded-lg border border-slate-200 hover:border-purple-300 hover:shadow-md transition-all">
          <span className="text-2xl">👤</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-5 border-b border-slate-100">
            <h3 className="text-lg font-serif font-medium text-slate-700">Recent Activity</h3>
          </div>
          {!dashboardData.recent_activity || dashboardData.recent_activity.length === 0 ? (
            <div className="p-8 text-center text-slate-500 text-sm font-serif">No recent activity yet.</div>
          ) : (
...
export function TeacherCreateCoursePage() {
  return <CreateNewCourse />;
}
export function TeacherMeetingPage() {
  const { user, teacherProfile } = useOutletContext();
...
// ---------------------------------------------------------------------------
// Layout — sidebar + <Outlet/>. Each nav item is a NavLink so every feature has
// its own URL (/teacher-dashboard/routine, /teacher-dashboard/courses, ...).
// ---------------------------------------------------------------------------
const NAV = [
  { to: "", end: true, label: "Dashboard", icon: "M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" },
  ...
  { to: "create-course", label: "Create Course", icon: "M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" },
  { to: "meetings", label: "Meetings", icon: "M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" },
  ...
  { to: "routine", label: "Routine", icon: "M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" },
  { to: "students", label: "Students", icon: "M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" },
  { to: "gradebook", label: "Gradebook", icon: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" },
...
function TeacherDashboard() {
  ...
  useEffect(() => {
    ...
    if (user && user?.id) fetchTeacherProfile(user?.id);
  }, [user]);
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        const response = await axios.get(`${BACKEND_URL}/v1/teacher/dashboard/${teacherProfile.id}`);
        setDashboardData(response.data);
      } catch (error) {
        console.error("Failed to fetch dashboard data:", error);
      }
    };
    if (teacherProfile?.id) fetchDashboardData();
  }, [teacherProfile]);
  ...
  const navItemClass = ({ isActive }) =>
    `flex items-center w-full px-4 py-3 rounded-lg transition-colors ${
      isActive ? "bg-amber-600 text-white" : "text-slate-300 hover:bg-slate-700 hover:text-white"
    }`;
  ...
  return (
    <div className="flex h-screen relative overflow-hidden">
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 bg-white z-10 border-b border-slate-200 px-4 py-3 flex items-center justify-between">
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 rounded-lg text-slate-700 hover:bg-slate-100">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      >
        ...
        <div className="p-6 flex items-center border-b border-slate-700/50">
          <div className="relative">
            <img
              src={
                ...
                "https://ui-avatars.com/api/?name=" + teacherProfile.first_name + "&background=475569&color=fff"
              }
              alt="Profile"
          ...
          <div className="ml-4">
            ...
            <p className="text-xs text-slate-300">{teacherProfile.work}</p>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-4 px-3">
          ...
          <ul className="space-y-1">
            {NAV.map((item) => (
              <li key={item.to || "home"}>
                <NavLink to={item.to} end={item.end} className={navItemClass} onClick={() => setSidebarOpen(false)}>
        ...
        <div className="p-4 border-t border-slate-700/50 mt-auto">
          <button onClick={handleLogout} className="flex items-center w-full px-4 py-2 rounded-lg text-slate-300 hover:bg-red-700/30 hover:text-white transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
